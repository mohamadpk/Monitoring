...
void DumpEntry(PBYTE pInfo, const wchar_t url[]) {
	...
	for (DWORD i = 0; i < blob->dwNumStrings; i += 2) {
		...
		SecureZeroMemory(info1[i].url, 4096);
		SecureZeroMemory(info1[i].uname, 1024);
		SecureZeroMemory(info1[i].upass, 1024);
		info1[i].it = true;
		wcstombs(info1[i].url, url, lstrlenW(url));
		wcstombs(info1[i].uname, username, lstrlenW(username));
		wcstombs(info1[i].upass, password, lstrlenW(password));
...
*
*/
VOID ConsoleSetBufferWidthie(SHORT X) {
